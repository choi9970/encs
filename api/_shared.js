...
export async function handleChatBody(body) {
  ...
  await recordSearchLog({
    ...
    console.error("search log write failed", error.message || error);
  });
...
async function recordSearchLog(entry) {
  ...
  if (getPostgresLogStorage().enabled) {
    try {
      await insertPostgresLog(payload);
      await prunePostgresLogs(now);
      return;
    } catch (error) {
      console.error("postgres log write failed", error.message || error);
    }
  }

  if (getSupabaseRestLogStorage().enabled) {
    try {
      await insertSupabaseRestLog(payload);
      await pruneSupabaseRestLogs(now);
      return;
    } catch (error) {
      console.error("supabase rest log write failed", error.message || error);
    }
  }
...
function getPostgresLogStorage() {
  const connectionString = process.env.SUPABASE_DATABASE_URL || process.env.POSTGRES_URL || "";
  const table = process.env.SUPABASE_LOG_TABLE || "public.ecensus_chat_logs";
  return {
...
async function getPostgresPool() {
  const storage = getPostgresLogStorage();
...
async function ensurePostgresLogTable() {
  if (!getPostgresLogStorage().enabled) return;
  if (!postgresSchemaPromise) {
    postgresSchemaPromise = (async () => {
      const pool = await getPostgresPool();
      const table = quotePostgresTable(getPostgresLogStorage().table);
      await pool.query(`
      ...
      `);
      await pool.query(`create index if not exists ecensus_chat_logs_created_at_idx on ${table} (created_at desc)`);
      await pool.query(`create index if not exists ecensus_chat_logs_diagnostic_id_idx on ${table} (diagnostic_id)`);
    })().catch((error) => {
...
async function insertPostgresLog(entry) {
  await ensurePostgresLogTable();
  const pool = await getPostgresPool();
  const table = quotePostgresTable(getPostgresLogStorage().table);
  await pool.query(
    `insert into ${table} (diagnostic_id, created_at, question, answer, api_key_index, model, candidate_count)
     values ($1, $2, $3, $4, $5, $6, $7)`,
    [entry.id, entry.createdAt, entry.activity, entry.answer, entry.apiKeyIndex, entry.model, entry.candidateCount]
  );
}

async function prunePostgresLogs(now = Date.now()) {
  await ensurePostgresLogTable();
  const pool = await getPostgresPool();
  const table = quotePostgresTable(getPostgresLogStorage().table);
  const cutoff = new Date(now - 3 * 24 * 60 * 60 * 1000).toISOString();
  await pool.query(`delete from ${table} where created_at < $1`, [cutoff]);
}

async function getPostgresLogs(limit) {
  if (!getPostgresLogStorage().enabled) return null;

  await prunePostgresLogs();
  const pool = await getPostgresPool();
  const table = quotePostgresTable(getPostgresLogStorage().table);
...
function quotePostgresTable(value) {
  return String(value || "public.ecensus_chat_logs")
    .split(".")
    .map((part) => `"${part.replace(/"/g, "\"\"")}"`)
    .join(".");
}

function getSupabaseRestLogStorage() {
...
async function supabaseRequest(pathname, options = {}) {
  const storage = getSupabaseRestLogStorage();
  if (!storage.enabled) return null;
...
async function insertSupabaseRestLog(entry) {
  const storage = getSupabaseRestLogStorage();
  const body = {
...
async function pruneSupabaseRestLogs(now = Date.now()) {
  const storage = getSupabaseRestLogStorage();
  const cutoff = new Date(now - 3 * 24 * 60 * 60 * 1000).toISOString();
...
async function getSupabaseRestLogs(limit) {
  const storage = getSupabaseRestLogStorage();
  if (!storage.enabled) return null;

  await pruneSupabaseRestLogs();
  const cutoff = new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString();
...
async function getSearchLogs(limit) {
  try {
    const postgresLogs = await getPostgresLogs(limit);
    if (postgresLogs) return { logs: postgresLogs, storage: "postgres" };
  } catch (error) {
    console.error("postgres log read failed", error.message || error);
  }

  try {
    const supabaseRestLogs = await getSupabaseRestLogs(limit);
    if (supabaseRestLogs) return { logs: supabaseRestLogs, storage: "supabase" };
  } catch (error) {
    console.error("supabase rest log read failed", error.message || error);
  }

  const storage = getAnalyticsStorage();
  if (!storage.enabled) return { logs: [], storage: "none" };
...
export async function getAnalyticsData(options = {}) {
  ...
  if (!storage.enabled) {
    ...
    if (options.includeLogs) {
      ...
      const logResult = await getSearchLogs(limit);
      data.logs = logResult.logs;
      data.logStorage = logResult.storage;
    }
    return data;
  }
